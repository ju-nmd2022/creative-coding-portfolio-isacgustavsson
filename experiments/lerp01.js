...
function setup() {
  ...
  let noise = new openSimplexNoise(); // Initialize with a seed
}

let motion_radius = 1.5;
...
function x1(t) {
  ...
  return (
    ...
    150 *
      noise(
        seed + motion_radius * cos(TWO_PI * t),
        motion_radius * sin(TWO_PI * t)
      )
  );
}
...
function draw() {
  ...
  for (let i = 0; i < m; i++) {
    ...
    let x = lerp(
      ...
      x2(t - delay_factor * (1 - tt)),
      tt
    );
    let y = lerp(
      y1(t - delay_factor * tt),
      y2(t - delay_factor * (1 - tt)),
      tt
    );
...
// ----------------------------------------------------------------------------
//
// The Following Code is the source of for Open Simplex Noise
//
// ----------------------------------------------------------------------------
